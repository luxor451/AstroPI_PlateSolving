...
impl TransformCoefficients {
    ...
    /// Averages the scale in X and Y directions for slight non-square pixels.
    pub fn scale(&self) -> f64 {
        let scale_x = (self.a1.powi(2) + self.a2.powi(2)).sqrt();
        let scale_y = (self.b1.powi(2) + self.b2.powi(2)).sqrt();
        (scale_x + scale_y) / 2.0
    }

    /// Returns the rotation angle in radians (used internally for convergence checks).
    pub fn rotation(&self) -> f64 {
        self.a2.atan2(self.a1)
    }

    /// Returns true if the image is mirrored (parity flip).
    /// This happens when the determinant of the rotation matrix is negative.
    ...
    /// 
    /// Position angle is measured from celestial North toward East.
    /// It indicates where the "top" of the image (negative Y in image coords) points.
    /// - 0° = North is up
    /// - 90° = East is up  
    /// - 180° = South is up
    /// - 270° = West is up
    /// 
    /// The transformation maps: (img_x, img_y) -> (cat_x, cat_y)
    /// where cat coords are in arcsec on the tangent plane (X=West, Y=North)
    /// 
    /// To find where image "up" (0, -1) points in sky coords:
    ///   sky_x = a1*0 + b1*(-1) = -b1
    ///   sky_y = a2*0 + b2*(-1) = -b2
    /// 
    /// Position angle = atan2(sky_x, sky_y) = atan2(-b1, -b2)
    pub fn position_angle(&self) -> f64 {
        // Where does image "up" (0, -1) point in the sky?
        // In tangent plane: +X is West, +Y is North
        // Position angle is from North toward East, so we need atan2(west_component, north_component)
        // But East = -West, so PA = atan2(-west, north) = atan2(b1, -b2)
        let mut angle = (self.b1).atan2(-self.b2).to_degrees();
...
pub struct PlateSolvingResult {
    ...
    /// Image rotation angle in degrees (0 = North up, positive = counter-clockwise)
    pub rotation_deg: f64,
    /// Image scale in arcseconds per pixel
    pub scale_arcsec_per_pixel: f64,
    /// Whether the image is mirrored (parity flip)
...
pub fn analyze_image(
    ...
    debug!("Found {} star quads.", star_quads.len());
...
pub fn solve_plate_with_options(
    ...
    for batch in search_positions.chunks(BATCH_SIZE) {
        ...
        for (iteration, match_result) in batch_results {
            ...
            debug!("Position {} (RA={:.4}°, Dec={:.4}°): {} matches", 
            ...
            if matched_count >= MIN_MATCHED_QUADS && 
               (best_matched_quads.len() < MIN_MATCHED_QUADS || iteration < solution_iteration) {
                info!("Found {} matches (>= {}), proceeding with solution.", 
    ...
    let (coeffs_x, coeffs_y, transform, optical_ra_deg, optical_dec_deg, final_matched_count) = if matched_count >= MIN_MATCHED_QUADS {
        ...
        for iteration in 0..MAX_STAR_MATCH_ITERATIONS {
            ...
            trace!("RMS residual: {:.2}\" Scale: {:.4}\"/pix Rotation: {:.2}°",
        ...
        debug!("Final transformation coefficients:");
        trace!("  X-axis: A1={:.6}, B1={:.6}, C1={:.6}", 
                     coeffs_x.0, coeffs_x.1, coeffs_x.2);
        trace!("  Y-axis: A2={:.6}, B2={:.6}, C2={:.6}", 
                 coeffs_y.0, coeffs_y.1, coeffs_y.2);
        debug!("  Scale: {:.4} arcsec/pixel", transform.scale());
        debug!("  Rotation: {:.4} degrees", transform.rotation().to_degrees());
        debug!("  Matched stars: {}", final_match_count);
        ...
        trace!("Image center RA:  {:.6}° ({:.6} rad)", center_ra.to_degrees(), center_ra);
    ...
    if coeffs_x.is_some() {
        ...
        if offset_deg > RE_CENTER_THRESHOLD_DEG {
            ...
            for re_iter in 0..MAX_RE_CENTER_ITERATIONS {
                ...
                trace!("Re-center iteration {}: projecting from RA={:.4}°, Dec={:.4}°",
                ...
                if new_match_result.matched_quads.len() >= MIN_MATCHED_QUADS {
                    ...
                    trace!("New center: RA={:.6}°, Dec={:.6}° (Δ={:.4}°, {:.4}°)",
    ...
    let (rotation_deg, scale_arcsec_per_pixel, is_mirrored) = if let Some(ref t) = final_transform {
        (t.position_angle(), t.scale(), t.is_mirrored())
    } else {
        (0.0, 0.0, false)
    };
